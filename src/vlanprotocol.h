...
public:
  ...
  CIncomingVLanSearchGame *RECEIVE_VLAN_SEARCHGAME( std::vector<uint8_t> data );
  CIncomingVLanGameInfo *RECEIVE_VLAN_GAMEINFO( std::vector<uint8_t> data );
...
private:
  bool AssignLength( std::vector<uint8_t> &content );
  bool ValidateLength( std::vector<uint8_t> &content );
};
